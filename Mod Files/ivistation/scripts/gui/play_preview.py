...
if not xbmc.Player().isPlayingVideo():
    ...
    if xbmc.getCondVisibility('Skin.HasSetting(synopsislayout)') or xbmc.getCondVisibility(
            'Skin.HasSetting(videolayout)'):
        ...
        try:
            if int(Current_Memory) >= 8:
                ...
                if Music_Playing == 1 and Played_Video == 1 and xbmc.getCondVisibility(
                        'Skin.HasSetting(Use_Startup_Playback)'):
                    time.sleep(1)
                    while True:
                        if not xbmc.Player().isPlayingVideo():
                            xbmc.executebuiltin('SetFocus(9000)')
                            ...
                            time.sleep(1)
                            xbmc.executebuiltin(
                                'PlayMedia(' + xbmc.getInfoLabel('Skin.String(Startup_Playback_Path)') + ')')
                            break
            else:
                dialog.ok("OOPS!", "There isn't enough free ram to play this video.",
                          "Current free ram = " + xbmc.getInfoLabel("system.freememory"))
