...
# TODO: Currently disabled, needs more testing. Implement later.
class GUI(xbmcgui.WindowXML):
    def onInit(self):
        global Moved_Selection
        Moved_Selection = 0
        if xbmc.getCondVisibility('Skin.HasSetting(gameloaded)') and xbmc.getCondVisibility(
                'Skin.HasSetting(lastromlist)'):
        ...
        if Moved_Selection == 0:
            FilePath = Media_Folder_Path + Current_System + '/videos/' + xbmc.getInfoLabel(
                'Container(9000).ListItem.Thumb')[:-4].lower()
    ...
    def onAction(self, action):
        ...
        if action.getButtonCode() == 270 or action.getButtonCode() == 271 or action.getButtonCode() == 272 or action.getButtonCode() == 273 or action.getButtonCode() == 262 or action.getButtonCode() == 263:
            ...
            while True:
                if xbmc.getCondVisibility('Window.IsVisible(1101)') or xbmc.getCondVisibility(
                        'Window.IsVisible(10000)') or xbmc.getCondVisibility('Skin.HasSettings(editmode)'):
                    xbmc.Player().stop()
                    break
                loop += 1
                time.sleep(0.05)
                if xbmc.getCondVisibility(
                        'System.IdleTime(' + Playback_Delay + ')') and Moved_Selection == 0 and not xbmc.getCondVisibility(
                        'Window.IsActive(1120)') and not xbmc.getCondVisibility('Window.IsActive(1101)'):
                    Moved_Selection = 1
                    FilePath = Media_Folder_Path + Current_System + '/videos/' + xbmc.getInfoLabel(
                        'Container(9000).ListItem.Thumb')[:-4].lower()
                    if os.path.isfile(FilePath + '.mp4'):
...
if __name__ == '__main__':
    ...
    try:
        ...
        pass
    if Music_Playing == 1 and xbmc.getCondVisibility(
            'Skin.HasSetting(Use_Startup_Playback)') and not xbmc.Player().isPlayingAudio():
