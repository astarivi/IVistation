...
class MadmabSettings(object):
    ...
    def _set_aspect_ratio(self, keep_ratio, res, flags):
        ...
        if resolution[0] in (1280, 1920):
            if keep_ratio:
                ...
                self.madmab_config["GENERAL"]["screenmaxx"] = str(screen_size_x)
                self.madmab_config["GENERAL"]["screenmaxx_" + mm_key] = str(screen_size_x)
                self.madmab_config["GENERAL"]["screenmaxy"] = str(resolution[1])
                self.madmab_config["GENERAL"]["screenmaxy_" + mm_key] = str(resolution[1])

                # Center the screen-x axis
                self.madmab_config["GENERAL"]["screenx"] = str(int((resolution[0] - screen_size_x) / 2.0))
                self.madmab_config["GENERAL"]["screenx_" + mm_key] = str(int((resolution[0] - screen_size_x) / 2.0))
                self.madmab_config["GENERAL"]["screeny"] = "0"
                self.madmab_config["GENERAL"]["screeny_" + mm_key] = "0"
                return

            # Stretched
            self.madmab_config["GENERAL"]["screenmaxx"] = str(resolution[0])
            self.madmab_config["GENERAL"]["screenmaxx_" + mm_key] = str(resolution[0])
            self.madmab_config["GENERAL"]["screenmaxy"] = str(resolution[1])
            self.madmab_config["GENERAL"]["screenmaxy_" + mm_key] = str(resolution[1])

            self.madmab_config["GENERAL"]["screenx"] = "0"
            self.madmab_config["GENERAL"]["screenx_" + mm_key] = "0"
            self.madmab_config["GENERAL"]["screeny"] = "0"
            self.madmab_config["GENERAL"]["screeny_" + mm_key] = "0"
            return
    ...
    def _handle_video_mode(self, dialog):
        ...
        self.madmab_config["GENERAL"]["video_mode"] = str(choice_index)
        self.madmab_config["GENERAL"]["init_video"] = "1"
        self.madmab_config["GENERAL"]["game_video_mode"] = "6"
        self.madmab_config["GENERAL"]["force_game_screen_size"] = "1"
        self.madmab_config["GENERAL"]["auto_game_screen_size"] = "4"
    ...
    def _show_submenu(self, title, option, dialog):
        ...
        current_index = int(self.madmab_config["GENERAL"][option])
        ...
        self.madmab_config["GENERAL"][option] = str(options.index(user_choice))
        self.should_save = True
    ...
    def save(self):
        ...
        with open(self.settings_path, "w") as config_file:
            self.madmab_config.write(config_file, space_around_delimiters=False)
