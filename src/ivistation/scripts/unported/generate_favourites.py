...
if os.path.isfile(Favs_List_XML):
	try:
		with open(Favs_List_XML) as file:
			for line in file:
				# try:
					# line = line.decode('unicode_escape').encode('utf-8') # Disabled as it causes issues with 007 games, so don't use Unicode characters in synopsis names and you will be fine.
				# except:
					# pass
				if '<favourites>'+Current_name+'|' in line:
					TMP = line.replace('<favourites>','').replace('</favourites>','')
					TMP = TMP.split('|')
					Display_Name = TMP[0]
					Emu_Path = TMP[1]
					Rom_Path = TMP[2]
