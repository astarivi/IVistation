...
if "PAL" in Resolution and os.path.isdir(os.path.join(xbmc.translatePath('special://skin/xml_sd_pal'))):
    ...
    Layout_Mode = 'sd_pal/'
    XML_Mode = 'xml_sd_pal/'
elif "NTSC" in Resolution and os.path.isdir(
        os.path.join(xbmc.translatePath('special://skin/xml_sd_ntsc'))) or "480p" in Resolution and os.path.isdir(
        os.path.join(xbmc.translatePath('special://skin/xml_sd_ntsc'))):
    print "SD NTSC Mode"
    Layout_Mode = 'sd_ntsc/'
...
Layout_File = xbmc.translatePath(
    'special://xbmc/ivistation/themes/' + HomeLayout + '/layouts/home/' + Layout_Mode + 'layout.xml')
...
if os.path.isfile(Layout_File):
    ...
    if os.path.isfile(Default_Layout_File):
        ...
        dialog.ok("ERROR!", "layout.xml is missing from", 'S:\\' + HomeLayout + '\\layouts\\home\\' + Layout_Mode + "",
                  "Reinstall this file to fix the issue.")
        Init_Run = ""
...
if os.path.isfile(System_List):
    ...
    dialog.ok("ERROR!", "system_list.xml is missing from", "S:\\simple\\system_list\\",
              "Reinstall this file to fix the issue.")
    Init_Run = ""
...
if Init_Run == "1":
    ...
    if xbmc.getCondVisibility('!Skin.HasSetting(firstrun)'):
    ...
    elif xbmc.getCondVisibility('Skin.HasSetting(gameloaded)') and xbmc.getCondVisibility(
        ...
        if xbmc.getCondVisibility('Skin.HasSetting(videolayout)'):
            ...
            xbmc.executebuiltin(
                'SetFocus(9000,' + xbmc.getInfoLabel('skin.string(lastrompos)') + ')')  # last system and rom
    else:
        xbmc.executebuiltin('ReplaceWindow(Home)')  # home screen
